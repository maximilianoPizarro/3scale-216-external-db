# Externalize PostgreSQL (10 → 15) in-cluster — Windows (Git Bash)

Same sequence as [01-externalize-postgresql.md](01-externalize-postgresql.md). This variant is for **Git Bash (MINGW64)**, not PowerShell.

Git Bash rewrites arguments that start with `/` (e.g. `/tmp`) to `C:/Users/.../AppData/Local/Temp`. `pg_dump` then tries to create the backup on the PC, not in the pod.

```
pg_dump: [custom archiver] could not open output file "C:/Users/.../AppData/Local/Temp/db_dump.backup"
```

`MSYS_NO_PATHCONV=1` disables that conversion. Remote-path `pg_dump` / `oc cp` calls are wrapped in `bash -c` or use explicit destination `/tmp/db_dump.backup`.
...
`tar: Removing leading '/' from member names` on `oc cp` is expected. The local file must start with magic `PGDMP` (not an error HTML or 0 bytes).
...
If `base64 -d` fails, use `base64 --decode`. Confirm the secret in `3scale-db` does not have empty `DB_USER` / `DB_PASSWORD` before the pod initializes the PVC.
...
Put the filename in the remote destination. A bare `oc cp ... :/tmp` gets rewritten again by Git Bash.
...
PostgreSQL 15 does not grant `CREATE` on `public` to the application user. Without this, the 2.16 jump `system-app-pre` fails with `permission denied for schema public`:
