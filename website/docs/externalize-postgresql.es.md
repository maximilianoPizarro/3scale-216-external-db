...
Sigue el capítulo oficial *PostgreSQL 10 upgrade — On-cluster*. Esta página resume el procedimiento en Linux con los manifiestos de este repositorio. Runbook completo: `docs/runbooks/01-externalize-postgresql.md`.
...
Desplegar **solo** PostgreSQL 15 primero — no el overlay `lab` completo (también crea Redis 7). Esperar `system-postgresql-external` en `1/1`.
...
### PostgreSQL 15: GRANT en schema `public`
...
Volver a subir **Redis embebido** (`backend-redis`, `system-redis`) antes del runbook de Redis. Restaurar el resto de réplicas y validar portales y APIs.

Tras confirmar, borrar el Deployment, PVC y Service de PostgreSQL embebido.
