...
Follow the official *PostgreSQL 10 upgrade — On-cluster* chapter. This page summarizes the Linux procedure with manifests from this repository. Full runbook: `docs/runbooks/01-externalize-postgresql.md`.
...
Capture replica counts **before** scaling to 0 (see runbook for the full list).

## 1. Scale down 3scale and the operator
...
Deploy **only** PostgreSQL 15 first — not the full `lab` overlay (that also creates Redis 7). Wait for `system-postgresql-external` at `1/1`.
...
### PostgreSQL 15: grant on schema `public`
...
Scale **embedded Redis** (`backend-redis`, `system-redis`) back up before the Redis runbook. Restore other replica counts, then validate portals and APIs.

After confirmation, delete the embedded PostgreSQL deployment, PVC, and service.
